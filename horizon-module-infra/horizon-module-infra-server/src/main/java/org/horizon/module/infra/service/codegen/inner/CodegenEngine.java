package org.horizon.module.infra.service.codegen.inner;
...
import org.horizon.framework.apilog.core.enums.OperateTypeEnum;
...
import org.horizon.framework.common.util.date.LocalDateTimeUtils;
...
import org.horizon.framework.excel.core.annotations.DictFormat;
import org.horizon.framework.excel.core.convert.DictConvert;
...
import org.horizon.module.infra.dal.dataobject.codegen.CodegenColumnDO;
import org.horizon.module.infra.dal.dataobject.codegen.CodegenTableDO;
import org.horizon.module.infra.enums.codegen.CodegenFrontTypeEnum;
import org.horizon.module.infra.enums.codegen.CodegenSceneEnum;
import org.horizon.module.infra.enums.codegen.CodegenTemplateTypeEnum;
import org.horizon.module.infra.enums.codegen.CodegenVOTypeEnum;
import org.horizon.module.infra.framework.codegen.config.CodegenProperties;
...
public class CodegenEngine {
    ...
    /**
     * 是否为 horizon-cloud 项目，用于解决 Boot 和 Cloud 的 api 模块兼容性问题
     *
     * true  - 需要有 horizon-module-xxx-api 模块
    ...
    public CodegenEngine() {
        ...
        this.cloudEnable = ClassUtils.isPresent("org.horizon.module.infra.framework.rpc.config.RpcConfiguration",
    ...
    private Map<String, String> getTemplates(Integer frontType) {
        ...
        if (Boolean.FALSE.equals(cloudEnable)) {
            SERVER_TEMPLATES.forEach((templatePath, filePath) -> {
                filePath = StrUtil.replace(filePath, "/horizon-module-${table.moduleName}-api", "");
                filePath = StrUtil.replace(filePath, "/horizon-module-${table.moduleName}-server", "");
    ...
    private static String javaModuleFilePath(String path, String module, String src) {
        return "horizon-module-${table.moduleName}/" + // 顶级模块
                "horizon-module-${table.moduleName}-" + module + "/" + // 子模块
                "src/" + src + "/java/${basePackage}/module/${table.moduleName}/" + path + ".java";
    }

    private static String mapperXmlFilePath() {
        return "horizon-module-${table.moduleName}/" + // 顶级模块
                "horizon-module-${table.moduleName}-server/" + // 子模块
    ...
    private static String vueFilePath(String path) {
        return "horizon-ui-${sceneEnum.basePackage}-vue2/" + // 顶级目录
                "src/" + path;
    }
    ...
    private static String vue3FilePath(String path) {
        return "horizon-ui-${sceneEnum.basePackage}-vue3/" + // 顶级目录
                "src/" + path;
    }

    private static String vue3VbenFilePath(String path) {
        return "horizon-ui-${sceneEnum.basePackage}-vben/" + // 顶级目录
                "src/" + path;
    }
